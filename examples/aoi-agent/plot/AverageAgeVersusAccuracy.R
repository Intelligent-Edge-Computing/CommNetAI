...
pdf("./plot/figures/average-age-versus-accuracy.pdf", width = 4.5, height = 4, pointsize = 12)

# 绘制图像
ggplot(data, aes(x = Age, y = Acc)) +
  geom_point( shape = 21, size = 0.5) +
  # geom_ribbon(aes(ymin = SuccessRate- 0.1, ymax = SuccessRate + 0.1), alpha = 0.5)+
  # geom_smooth(method = "lm", formula = y ~ poly(x, 2), se = TRUE, fill = "grey") +
  labs(
       x = "Average Age (s)",
       y = "Accuracy (%)") +
  theme_minimal()
